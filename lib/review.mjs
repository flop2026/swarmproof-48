...
function canonicalTransportTime(value) {
...
function strictLiveProtocolRecords(roomData, context, observedAt) {
  ...
  for (const message of roomData.messages) {
    ...
    try {
      ...
      const sourceTs = canonicalTransportTime(message.ts);
      if (
...
function analyzedReviewRoom(context, roomData, observedAt) {
  ...
  return {
    graph,
    liveEventIds: new Set(liveRecords.map(record => sha256Hex(record.envelope))),
  };
}

function latestValidReview(context, roomData, reviewerDid, observedAt) {
