...
import { canonicalUtcTimestamp } from "./collector.mjs";
...
function validateProjectContext(config, manifest, { allowComplete = false } = {}) {
  ...
  assert(
    config.state === "active" || (allowComplete && config.state === "complete"),
    allowComplete
      ? "Review read-back requires an active or complete event."
      : "Reviews may only be prepared while the event is active.",
  );
...
function validatePublicMetadata(config, report, status, now, { allowComplete = false } = {}) {
  ...
  assert(
    report.event_state === "active" || (allowComplete && report.event_state === "complete"),
    "Public event state is unsupported for this review operation.",
  );
  assert(status.state === report.event_state, "Public report and status states differ.");
  ...
  if (config.state === "active") {
    ...
    assert(allowComplete, "Public event state is not active.");
    assert(nowMs >= Date.parse(config.ends_at), "Completed review read-back cannot precede the event boundary.");
  }
...
export function validateAndBindPublicReview({
  ...
  validateProjectContext(config, manifest, { allowComplete });
  const publicMetadata = validatePublicMetadata(config, report, status, now, { allowComplete });
  assert(
...
export async function fetchPublicReviewDocuments({
  ...
  const fixedOrigin = new URL(REVIEW_PUBLIC_ORIGIN);
  assert(
...
export async function readSecureReviewKey(path) {
  ...
  try {
    ...
    assert(Buffer.byteLength(content, "utf8") <= 16_384, "Private key size changed during its safe read.");
    return content;
  } finally {
    await handle.close();
  }
...
function strictLiveProtocolRecords(roomData, context, observedAt) {
  validateRoomData(roomData, context.config.build_room);
  const observed = new Date(observedAt);
  assert(!Number.isNaN(observed.getTime()), "Build-room observation time is invalid.");
  const canonicalObservedAt = observed.toISOString();
  ...
  for (const message of roomData.messages) {
    ...
    try {
      ...
      if (
        ...
        source_ts: sourceTs,
        source_room: context.config.build_room,
...
function mergeReviewRecords(existing, incoming, context) {
  const options = protocolOptions(context.config, context.manifest);
  const observations = new Map();
  for (const record of [...existing, ...incoming]) {
    const event = verifyEnvelope(record.envelope, options);
    const group = observations.get(event.event_id) ?? [];
    group.push(record);
    observations.set(event.event_id, group);
  }
  const merged = [];
  const sequenceOwners = new Map();
  for (const [eventId, records] of observations) {
    const observedAt = records.map(record => record.observed_at).sort()[0];
    canonicalTime(observedAt, `Merged observation ${eventId}.observed_at`);
    const sequenced = records.filter(record => Number.isSafeInteger(record.source_seq));
    let sourceSeq = null;
    let sourceTs = null;
    if (sequenced.length > 0) {
      sourceSeq = Math.min(...sequenced.map(record => record.source_seq));
      const sourceTimes = [...new Set(sequenced
        .filter(record => record.source_seq === sourceSeq)
        .map(record => record.source_ts)
        .filter(value => value !== null))];
      sourceTs = sourceTimes.length === 1 ? sourceTimes[0] : null;
    } else {
      const sourceTimes = records.map(record => record.source_ts).filter(value => value !== null).sort();
      sourceTs = sourceTimes[0] ?? null;
    }
    if (sourceTs !== null) canonicalTime(sourceTs, `Merged observation ${eventId}.source_ts`);
    if (sourceSeq !== null) {
      const previousOwner = sequenceOwners.get(sourceSeq);
      assert(!previousOwner || previousOwner === eventId, "Merged build-room records contain a conflicting source sequence.");
      sequenceOwners.set(sourceSeq, eventId);
    }
    merged.push({
  ...
  return merged.sort((left, right) => left.envelope.localeCompare(right.envelope));
}

export function analyzedReviewRoom(context, roomData, observedAt) {
  const archivedLastSeq = context.report?.build_room?.last_seq;
  assert(Number.isSafeInteger(archivedLastSeq) && archivedLastSeq >= 0, "Trusted snapshot build-room cursor is invalid.");
  validateRoomData(roomData, context.config.build_room);
  if (context.config.state === "active") {
    if (roomData.messages.length === 0) {
      assert(archivedLastSeq === 0, "Live build-room tail does not overlap the trusted snapshot cursor.");
    } else {
      assert(
        roomData.last_seq >= archivedLastSeq && roomData.first_seq <= archivedLastSeq + 1,
        "Live build-room tail does not overlap or continue the trusted snapshot cursor.",
      );
    }
  }
  const liveRecords = strictLiveProtocolRecords(roomData, context, observedAt);
  const records = mergeReviewRecords(context.records, liveRecords, context);
  const graph = analyzeEventSemantics(records, {
    allowedRepositories: [context.config.repository],
    allowedTasks: new Set(context.manifest.tasks.map(task => task.id)),
    coordinatorDid: context.config.coordinator_did,
    startsAt: context.config.starts_at,
    endsAt: context.config.ends_at,
  });
  return {
    graph,
    liveEventIds: new Set(liveRecords.map(record => record.event_id)),
  };
}

export function latestValidReview(context, roomData, reviewerDid, observedAt) {
  const { graph } = analyzedReviewRoom(context, roomData, observedAt);
...
export function maximumSafeRoomNonce(roomData, did) {
  let maximum = 0n;
  for (const message of roomData.messages) {
...
function existingReview(context, roomData, reviewerDid, observedAt) {
  const latest = latestValidReview(context, roomData, reviewerDid, observedAt);
  return latest?.payload.review.verdict === context.protocolVerdict ? latest : null;
}
...
export function prepareSignedReview({ context, roomData, privateKeyPem, now = new Date() }) {
  ...
  assert(reviewerDid !== context.config.coordinator_did, "A review must use a DID different from the project controller.");
  const duplicate = existingReview(context, roomData, reviewerDid, now);
...
export function findValidObservedReview({ context, roomData, eventId, observedAt = new Date() }) {
  const analyzed = analyzedReviewRoom(context, roomData, observedAt);
  if (!analyzed.liveEventIds.has(eventId) || !analyzed.graph.validReviewIds.has(eventId)) return null;
  return analyzed.graph.eventById.get(eventId) ?? null;
}
...
async function observeReview({ context, eventId, fetchImpl, technocoreOrigin }) {
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    ...
    const observed = findValidObservedReview({ context, roomData, eventId, observedAt: new Date() });
