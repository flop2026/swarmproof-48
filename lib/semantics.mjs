...
const WINDOW_END_TYPES = new Set(["TASK", "CLAIM", "RESULT", "REVIEW", "PROMOTE"]);
const DAG_EVIDENCE_TYPES = new Set(["TASK", "CLAIM", "RESULT", "REVIEW", "PROMOTE"]);
...
export function analyzeEventSemantics(records, options = {}) {
  ...
  for (const event of verified) {
    ...
    if (
      ...
      || event.payload.did === target.payload.did
      || (options.coordinatorDid && event.payload.did === options.coordinatorDid)
    ) {
      addReason(state, event.event_id, "review-target-or-key-distinctness-is-invalid");
      continue;
    }
  ...
  for (const event of [...verified].sort(compareEventChronology)) {
    ...
    if (promotedResultIds.has(targetId)) {
      addReason(state, event.event_id, "duplicate-promotion-for-result");
      continue;
    }
