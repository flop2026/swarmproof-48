...
export function analyzeEventSemantics(records, options = {}) {
  ...
  for (const event of verified) {
    ...
    const priorCrossKeyPass = [...(validPassReviews.get(targetId) ?? [])].some(reviewerDid => {
      if (reviewerDid === target.payload.did) return false;
      return verified.some(review => (
        validReviewIds.has(review.event_id)
        && review.payload.did === reviewerDid
        && review.payload.review?.target_event_id === targetId
        && review.payload.review.verdict === "PASS"
        && parentPrecedes(review, event)
      ));
    });
