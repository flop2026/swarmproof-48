...
export function assessSourceCommitComparison(comparison, sourceCommit) {
  ...
  assert(COMMIT_RE.test(comparison.head_commit?.sha ?? ""), "GitHub comparison head is invalid.");
  return { eligible: true, main_commit: comparison.head_commit.sha };
}
...
export function assessCheckpointInputs({
  ...
  assert(sample.schema === "swarmproof-network-sample-v1", "Unsupported network sample schema.");
