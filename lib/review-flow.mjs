...
const TRANSPORT_KEYS = new Set([
  ...
  "event_id", "envelope", "transport_signature",
]);
...
export function createReviewTransport({ packet, request, privateKey }) {
  ...
  const transport = {
    ...
    target_packet_sha256: reviewDocumentSha256(packet),
    signing_request_sha256: reviewDocumentSha256(request),
    room: packet.project.room,
    did: reviewerDid,
...
function equivalentLatestReview(context, roomData, transport, observedAt) {
  const latest = latestValidReview(context, roomData, transport.did, observedAt);
  if (!latest || latest.payload.review.target_event_id !== context.target.event_id) return null;
  return latest;
}
...
export async function readBackReviewTransport({
  ...
  const observed = findValidObservedReview({
    context,
    roomData,
    eventId: transport.event_id,
    observedAt: new Date(),
  });
  return { status: observed ? "observed" : "not-observed", observed };
}
...
export async function postReviewTransport({
  ...
  const alreadyObserved = findValidObservedReview({
    context,
    roomData,
    eventId: transport.event_id,
    observedAt: new Date(),
  });
  if (alreadyObserved) return { status: "already-observed", observed: alreadyObserved };
  const latest = equivalentLatestReview(context, roomData, transport, new Date());
  if (latest) {
    if (latest.payload.review.verdict === request.payload.review.verdict) {
      return { status: "equivalent-review-exists", observed: latest };
    }
    assert(
  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    ...
    const observed = findValidObservedReview({
      context,
      roomData: observedRoom,
      eventId: transport.event_id,
      observedAt: new Date(),
    });
    if (observed) return { status: "posted-and-observed", observed };
