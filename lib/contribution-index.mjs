...
export const CONTRIBUTION_INDEX_MAX_HISTORY = 128;
...
export function verifyContributionIndexReplacement(previousInput, nextLink, options = {}) {
  assert(
    typeof previousInput === "string",
    "Previous contribution index must be canonical serialized signed text.",
  );
  assertExactObject(
    nextLink,
...
export function verifyContributionIndexHistory(previousInputs, currentInput, options = {}) {
  assert(Array.isArray(previousInputs), "Contribution-index history must be an array.");
  assert(previousInputs.length >= 1, "Contribution-index history must include at least sequence 1.");
  assert(
    previousInputs.length < CONTRIBUTION_INDEX_MAX_HISTORY,
    "Contribution-index history limit exceeded.",
  );
  assert(
    typeof currentInput === "string",
    "Current contribution index must be canonical serialized signed text.",
  );
  previousInputs.forEach((input, index) => {
    assert(
      typeof input === "string",
      `Previous contribution index ${index + 1} must be canonical serialized signed text.`,
    );
  });

  const indexes = [...previousInputs, currentInput].map(input => (
    verifyContributionIndex(input, options)
  ));
  assert(
    indexes[0].document.payload.sequence === "1",
    "Complete contribution-index history must begin at sequence 1.",
  );
  for (let index = 1; index < indexes.length; index += 1) {
    const previous = indexes[index - 1];
    const current = indexes[index];
    assert(
      BigInt(current.document.payload.sequence)
        === BigInt(previous.document.payload.sequence) + 1n,
      "Contribution-index history sequence must increment by exactly one.",
    );
    assert(
      current.document.payload.previous_index_sha256 === previous.index_sha256,
      "Contribution-index history link does not name the preceding index.",
    );
  }
  ...
  return {
    ...
    first_sequence: indexes[0].document.payload.sequence,
    current_sequence: current.document.payload.sequence,
    first_index_sha256: indexes[0].index_sha256,
...
export function deriveContributionFacets(indexInput, project) {
  assert(
    typeof indexInput === "string",
    "Contribution facets require canonical serialized signed index text.",
  );
  const index = verifyContributionIndex(indexInput, {
    expectedController: project.config.coordinator_did,
  });
  const payload = index.document.payload;
